...
// Legacy weak hash (djb2) — kept only to verify hashes created before v2 scheme
...
async function pwHash(pw: string): Promise<string> {
...
async function pwVerify(stored: string | undefined, pw: string): Promise<boolean> {
  ...
  persist(
    (set, get) => ({
      ...
      saveFile: async (id) => {
        ...
        if (!note) { get().showToast(t('toast_note_not_found')); return false; }
        ...
        if (isMobile) {
          try {
            ...
            if (!ok) {
              get().showToast(t('toast_grant_storage'));
        ...
        if (!note.filePath) {
          ...
          if (isMobile) {
            get().showToast(t('toast_choose_save_location'));
            set({ fileBrowserOpen: true, fileBrowserNoteId: id });
            return false;
          }
          get().showToast(t('toast_opening_save_dialog'));
          try {
            const uri = await invoke<string | null>('save_file_dialog', {
              fileName: `${note.title || 'untitled'}.txt`,
              content: text,
            });
            if (!uri) { get().showToast(t('toast_save_cancelled')); return false; }
            get().updateNote(id, { filePath: uri });
            get().showToast(t('toast_saved'));
            return true;
          } catch (e) {
            const msg = typeof e === 'string' ? e : String(e);
            get().addLog('saveFile: ' + msg);
            get().showToast(t('toast_save_failed') + msg, 'error');
            return false;
          }
        }
        ...
        if (newPath !== note.filePath) {
          try {
            ...
            get().showToast(t('toast_rename_failed') + msg, 'error');
            return false;
          }
      ...
      finishFileBrowserSave: async (id, path) => {
        ...
        try {
          ...
          get().showToast(t('toast_save_failed') + msg);
          set({ fileBrowserOpen: false, fileBrowserNoteId: null });
          return false;
        }
      },
      ...
      saveFileAs: async (id, ext) => {
        ...
        if (note.filePath) {
          // File-backed notes keep their raw content byte-for-byte
      ...
      exportData: async () => {
        try {
          ...
          if (!uri) return;
          get().showToast(t('toast_exported'));
        } catch (e) {
          const msg = typeof e === 'string' ? e : String(e);
          get().addLog('exportData: ' + msg);
          get().showToast(t('toast_export_failed') + msg, 'error');
        }
      },
      ...
      importData: async () => {
        ...
        try {
          ...
          if (!data.notes || !data.tasks) {
            get().showToast(t('toast_invalid_backup'), 'error');
            return;
          }
          set({
            notes: data.notes,
          ...
          get().showToast(t('toast_imported'));
        } catch {
          get().showToast(t('toast_import_failed'), 'error');
        }
      },
      ...
      runConversion: async () => {
        ...
        try {
          ...
          if (inputCat === 'image' && outputCat === 'image') {
            ...
            get().showToast(t('toast_converted') + outFmt);
            return;
          }
          ...
          get().showToast(t('toast_unsupported_conversion'), 'error');
          set({ converterLoading: false });
        } catch (e) {
          get().showToast(t('toast_conversion_failed') + e, 'error');
          set({ converterLoading: false });
        }
      },
