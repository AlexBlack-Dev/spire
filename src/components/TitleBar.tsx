...
export default function TitleBar() {
  ...
  const menuItems = [
    ...
    { label: t('private'),   action: () => { setViewMode('private');   close(); } },
    { sep: true },
          ...
          {menuOpen && (
            ...
            >
              {menuItems.map((item, i) =>
                item.sep ? (
                  <div key={i} style={{ height: 1, background: 'var(--border-default)', margin: '3px 4px' }} />
                ) : item.submenu ? (
                  <div key={i} style={{ position: 'relative' }}>
                    <button
                      ...
                      style={{
                        ...
                        color: item.disabled ? 'var(--text-disabled)' : 'var(--text-secondary)',
                  ...
                  >
                    <span>{item.label}</span>
                    {item.shortcut && (
                      <span style={{ fontSize: 11, color: 'var(--text-disabled)', marginLeft: 20 }}>{item.shortcut}</span>
                    )}
                  </button>
                )
              )}
