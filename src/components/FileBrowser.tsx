  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: `0 ${dim.sp2}px`, flexShrink: 0 }}>
        ...
        <span style={{ fontSize: dim.textMd, fontWeight: 700, color: 'var(--text-primary)' }}>{t('fb_save_to_folder')}</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: `${dim.sp2}px ${dim.sp4}px ${dim.sp7}px` }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: dim.sp8, color: 'var(--text-disabled)', fontWeight: 600 }}>{t('fb_loading')}</div>
        ) : entries.length === 0 ? (
          <div style={{ textAlign: 'center', padding: dim.sp8, color: 'var(--text-disabled)', fontWeight: 600 }}>{t('fb_empty')}</div>
        ) : (
          entries.map((e) => (
      ...
      <div style={{ padding: `${dim.sp4}px ${dim.sp6}px calc(env(safe-area-inset-bottom, 8px) + ${dim.sp4}px)`, flexShrink: 0 }}>
        <motion.button whileTap={{ scale: 0.95 }} onClick={handleSave} style={{
          ...
          {t('fb_save_here', { name: noteTitle || 'untitled' })}
        </motion.button>
      </div>
    </div>
  );
